...
function getRandomQuote() {
  ...
  if (randNum == quotenumber) {
    ...
    // if else statment to make sure there is no repeated quotes
    quotenumber = randNum;
  }
...
function newClick() {
  for (let i = 1; i <= 3; i++) {
    // will only loop 3 times
    if (num.length > 2) {
      // makes sure there is only 3 numbers in the array at all times
  ...
  document.querySelector(
    "body"
  ).style.backgroundColor = `rgb(${num[0]}, ${num[1]}, ${num[2]})`;
}
